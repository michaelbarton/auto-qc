...
def test_is_variable_path_valid_with_valid_path():
    ...
    assert variable.is_variable_path_valid(analysis, path)


def test_is_variable_path_valid_with_invalid_path():
    path = ":unknown/metric_1"
    analysis = {"ref": {"metric_1": 2}}
    assert not variable.is_variable_path_valid(analysis, path)


def test_get_variables_with_no_nesting():
    qc_node = ["<", ":ref/metric_1", 1]
    assert variable.get_variable_names(qc_node) == [":ref/metric_1"]
